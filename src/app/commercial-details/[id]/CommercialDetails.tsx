...
const CommercialDetails = ({params}) => {
  const { id } = params;
  ...
  const dataFields = [
    ...
    { label: "Built Up Area / Carpet Area / Super Built Up Area", value: reportData?.built_up_area_carpet_area_super_built_up_area },
  ...
  useEffect(() => {
    const getUserData = async () => {
      ...
      try {
        ...
        setReportData({...response?.data?.report, DV, RV , final_valuation, building_value});
              ...
              <div>
                <div className="mb-2 bg-mainColor">
                  <p className="pb-[12px] pt-[12px] text-center font-serif text-2xl font-semibold text-white">
                    Confirm Commercial Details
                  </p>
                </div>
                <div className="h-[80%] bg-gray-200 max-[700px]:p-[8px]">
                  <div className="pt-15 grid w-full grid-cols-3 gap-14 pb-10 pl-10 pt-10 text-center max-[700px]:ml-0 max-[700px]:flex max-[700px]:flex-col max-[700px]:pl-0 ">
                    <div className="mr-2 text-[16px] font-semibold text-black max-[700px]:text-[20px] max-[400px]:mb-[8px]">
                      Carpet Area{" "}
                      <p className="text-black">{reportData?.carpet_area}</p>
                    </div>
                    <div className="mr-2 border-l-4 border-green-950 text-[16px] font-semibold text-black max-[700px]:border-l-0 max-[700px]:text-[22px] max-[400px]:mb-[8px]">
                      ...
                      <p className="text-black">
                        {reportData?.super_built_up_area}
                      </p>
                    </div>
                    <div className="mr-2 border-l-4 border-green-950 text-[16px] font-semibold text-black max-[700px]:border-l-0 max-[700px]:text-[22px] max-[400px]:mb-[8px]">
                      Loading<p className="text-black">{reportData?.loading}</p>
                    </div>
                  </div>
